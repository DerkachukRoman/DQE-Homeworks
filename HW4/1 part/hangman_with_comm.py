...
# one more parameter (last_letter) was added
def antycheat(user_input: str, last_user_input: str, initial_len_of_input: int, last_letter: str) -> typing.Tuple[str, int]:
    ...
    while last_letter not in corrected_input: #add checking if letter in user_input
        print("Что-то тут не так.")
        print("В прошлый раз была другая буква.")
        print(f"А именно {last_letter}.")
...
#Инициализируем все перед началом игры
    ...
    guesses: str = ""
    current_word: str = ""
    likeliest_letter: typing.Tuple[str, float] = ("_", 100) #letter is absent for first iteration
