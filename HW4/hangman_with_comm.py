...
def antycheat(user_input: str, last_user_input: str, initial_len_of_input: int, last_letter: str) -> typing.Tuple[str, int]:
    ...
    has_differences: bool = all(differences) or reduce(lambda x, y: x != y, differences) or last_letter not in user_input
...
#Инициализируем все перед началом игры
    ...
    while is_playing:
        if was_correct:
            ...
            current_word, len_of_word = antycheat(current_word, last_word, len_of_word, likeliest_letter)
